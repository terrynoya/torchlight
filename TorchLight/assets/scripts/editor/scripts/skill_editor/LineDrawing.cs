...
public class LineDrawing
{
    ...
    public static void DrawBezierLine(Vector2 start, Vector2 startTangent, Vector2 end, Vector2 endTangent, Color color, float width, int segments)
    {
        ...
        for (int i = 1; i <= segments; ++i)
        {
            ...
            LineDrawing.DrawLine(lastV, v, color, width);
            lastV = v;
        }
    }

    public static void DrawCurveFromTo(Rect wr, Rect wr2, Color color, int width)
    {
        DrawBezierLine(
            ...
            wr2.center,
            new Vector2(wr2.x - Mathf.Abs(wr2.x - (wr.x + wr.width)) / 2, wr2.y + 3 + wr2.height / 2), color, width, 20);
    }
